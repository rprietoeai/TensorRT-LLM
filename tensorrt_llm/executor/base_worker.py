        ...
        try:
            ...
            if request.query_token_ids is not None:
                ...
                if self._is_pytorch_backend and result_wait_queue is not None:
                    ramon.log("engine queing request with query token ids pytorch backend without waiting")
                    req_id = self.engine.enqueue_request(
                        executor_request,
                        request.query_token_ids,
                        result_wait_queue=result_wait_queue)
                else:
                    ramon.log("engine queing request")
                    req_id = self.engine.enqueue_request(
                        executor_request, request.query_token_ids)
            else:
                if self._is_pytorch_backend and result_wait_queue is not None:
                    ramon.log("engine queing request without query_token_ids with backedn and with waiting")
                    req_id = self.engine.enqueue_request(
                        executor_request, result_wait_queue=result_wait_queue)
                else:
                    ramon.log("engine queing request without query_token_ids")
